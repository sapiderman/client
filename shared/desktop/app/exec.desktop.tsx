...
export default function(
  ...
  if (!path) {
    logger.info('Exec path not available:', path)
    if (callback) callback(null, false, '', '')
    return
  }
  if (runModeOnly && runMode !== runModeOnly) {
    logger.info('Exec path not available for this run mode: %s != %s', runModeOnly, runMode)
    if (callback) callback(null, false, '', '')
    return
  }
  ...
  fs.access(path, fs.X_OK, function(err) {
    if (err) {
      logger.info('Exec path not found (or accessible as executable):', path)
      if (callback) callback(null, false, '', '')
      return
    }
